...
class locationController extends Controller
{
    ...
    $lokasi_toko = DB::table('lokasi_toko')->get();
		return view('geolocation/location',['lokasi_toko'=>$lokasi_toko]);
    }
    
    public function titikAwal(){
		return view('geolocation/inputTitikAwal');
    }

    public function titikKunjungan(){
		return view('geolocation/titikKunjungan');
    }
    ...
    public  function cetak(){ 
      ...
      $pdf =  PDF::loadView  ('geolocation/cetakBarcode',  compact('lokasi', 'no')); 
      $pdf->setPaper("f4"); 
      return $pdf->stream(); 
    }
     ...
     public  function cetakBarcodeId($id){ 
        $lokasi = DB::table('lokasi_toko',$id)->where('barcode',$id)->get();
        $no = 1; 
        // $brid=$id; 
        $paper_width = 793.7007874; // 38 mm
        $paper_height = 623.62204724; // 18 mm
        $paper_size = array(0, 0, $paper_width, $paper_height);
        $pdf =  PDF::loadView  ('geolocation/cetakBarcode_id', compact('lokasi', 'no')); 
        $pdf->setPaper('letter'); 
        return $pdf->stream(); 
    }
   ...
   public function alert($id){
        $barang = DB::table('lokasi_toko')->where('barcode',$id)->get();
        return json_encode($lok);
    }
